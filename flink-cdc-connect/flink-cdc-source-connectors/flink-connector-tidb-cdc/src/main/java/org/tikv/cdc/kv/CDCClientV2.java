...
public class CDCClientV2 {
  ...
  public void start(final long startTs) {
    LOGGER.debug(
        "Start cdc client at time {},database {} table {} listening.", startTs, dbName, tableName);
          ...
          CompletableFuture.runAsync(
              () -> { // try insert retry Mechanism
                ...
                if (!tableInfoOptional.isPresent()) {
                  LOGGER.error("Get tableInfo for {}.{} failed!", dbName, tableName);
                  throw new ClientException(
                      String.format("Get tableInfo for %s.%s failed.", dbName, tableName));
                }
                KeyRange keyRange =
                    TableKeyRangeUtils.getTableKeyRange(tableInfoOptional.get().getId());
                ...
                for (RegionStateManager.SingleRegionInfo sri : singleRegionInfos) {
                  requestRegionToStore(sri, startTs, tableInfoOptional.get().getId());
                }
                while (isRunning()) {
                  ...
                  for (EventListener eventListener : listeners) {
                    ...
                    if (regionFeedEvent.getRawKVEntry() != null) {
                      eventListener.notify(
                          output(regionFeedEvent.getRawKVEntry(), tableInfoOptional));
                    }

                    if (regionFeedEvent.getResolved() != null) {
                      for (RegionKeyRange kr : regionFeedEvent.getResolved().getKeyRanges()) {
                        // todo comparable
                        tsTracker.forward(
                            kr.getRegionId(),
                            kr.getKeyRange(),
                      ...
                      output(
                          new RawKVEntry.Builder()
                              ...
                              .setRegionId(regionFeedEvent.getRegionId())
                              .build(),
                          tableInfoOptional);
                    }
                  }
  ...
  private PolymorphicEvent output(RawKVEntry raw, Optional<TiTableInfo> tableInfoOptional) {
    if (raw.getCrts() < this.resolvedTs.get()
        || (raw.getCrts() == this.resolvedTs.get() && OpType.Resolved.equals(raw.getOpType()))) {
      LOGGER.warn(
  ...
  public void close() {
    try {
      ...
      if (!clientExecutor.awaitTermination(clientClosedTimeoutSeconds, TimeUnit.SECONDS)) {
        LOGGER.error("Failed to close the cdc client in {} seconds.", clientClosedTimeoutSeconds);
      }
    } catch (InterruptedException e) {
  ...
  private void requestRegionToStore(
    ...
    if (!storeStreamCache.containsKey(storeAddr) || streamClient.getIsCanceled()) {
      ...
      LOGGER.info(
          "creating new stream {} to store {} to send request", stream.getStreamId(), storeAddr);
      streamClient = stream;
    }
  ...
  private void receiveFromStream(
          ...
          public void onNext(Cdcpb.ChangeDataEvent event) {
            ...
            if (!event.getEventsList().isEmpty()) {
              if (event.getEventsList().get(0).hasEntries()) {
                long commitTs =
                    event.getEventsList().get(0).getEntries().getEntries(0).getCommitTs();
    ...
    StreamObserverAdapter.CallFactory<Cdcpb.ChangeDataRequest, Cdcpb.ChangeDataEvent> callFactory =
        (observer) -> stream.getAsyncStub().eventFeed(observer);
  ...
  private void deleteStream(EventFeedStream deleteStreamClient) {
    EventFeedStream regionStreamClientInMap = storeStreamCache.get(deleteStreamClient.getAddr());
    if (regionStreamClientInMap == null) {
      LOGGER.warn(
          "Delete stream {} failed, stream not found,ignore it", deleteStreamClient.getAddr());
      return;
    }
    if (regionStreamClientInMap.getStreamId() != deleteStreamClient.getStreamId()) {
      LOGGER.warn(
          "Delete stream {} failed, stream id mismatch,ignore it", deleteStreamClient.getAddr());
      return;
    }
  ...
  private void sendRegionChangeEvent(List<Cdcpb.Event> events, RegionWorker worker) {
            ...
            .mapToObj(
                i ->
                    IntStream.range(0, events.size())
                        .mapToObj(j -> new RegionStatefulEvent()) // 假设有一个无参构造函数
                        .collect(Collectors.toList()))
            .collect(Collectors.toList());
    ...
    for (Cdcpb.Event event : events) {
      ...
      regionStatefulEeventList
          .get(slot)
          .add(new RegionStatefulEvent.Builder().setEvent(event).setRegionFeedState(state).build());
    }
