    ...
    public void start(final long startTs) {
        ...
        if (started.compareAndSet(false, true)) {
            CompletableFuture.runAsync(
                    () -> {
                        while (isRunning()) { // try insert retry Mechanism
                            ...
                            while (isRunning()) {
                                for (EventListener eventListener : listeners) {
                                    RegionFeedEvent rfe = eventsBuffer.poll();
                                    if (rfe == null) {
                                        continue;
                                    }
                                    ...
                                    rfe.setTableInfo(
                                            this.getTableInfo(this.dbName, this.tableName).get());
                                    if (rfe.getResolved() != null) {
                                        resolvedTs.getAndSet(rfe.getResolved().getResolvedTs());
                                    }
                                    // todo
                                    eventListener.notify(rfe.getRawKVEntry());
                                }
                            }
                        }
                    },
