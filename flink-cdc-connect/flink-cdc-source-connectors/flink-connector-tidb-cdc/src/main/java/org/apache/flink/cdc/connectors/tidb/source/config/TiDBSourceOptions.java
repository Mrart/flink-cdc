...
public class TiDBSourceOptions extends JdbcSourceOptions {
    ...
    public static final ConfigOption<Long> TIKV_GRPC_INGEST_TIMEOUT =
            ConfigOptions.key(ConfigUtils.TIKV_GRPC_INGEST_TIMEOUT).longType().noDefaultValue();

    public static final ConfigOption<Long> TIKV_GRPC_FORWARD_TIMEOUT =
            ConfigOptions.key(ConfigUtils.TIKV_GRPC_FORWARD_TIMEOUT).longType().noDefaultValue();

    public static final ConfigOption<Long> TIKV_GRPC_WARM_UP_TIMEOUT =
            ConfigOptions.key(ConfigUtils.TIKV_GRPC_WARM_UP_TIMEOUT)
                    .longType()
                    .noDefaultValue()
                    .withDescription("tikv.grpc.warm_up_timeout_in_ms");
    ...
    public static final ConfigOption<Integer> TIKV_INDEX_SCAN_BATCH_SIZE =
            ConfigOptions.key(ConfigUtils.TIKV_INDEX_SCAN_BATCH_SIZE)
                    .intType()
                    .noDefaultValue()
                    .withDescription("tikv.index.scan_batch_size");
    ...
    public static TiConfiguration getTiConfiguration(
        ...
        configuration
                .getOptional(TIKV_PD_FIRST_GET_MEMBER_TIMEOUT)
                .ifPresent(tiConf::setPdFirstGetMemberTimeout);
        ...
        configuration
                .getOptional(TIKV_GRPC_KEEPALIVE_TIMEOUT)
                .ifPresent(tiConf::setKeepaliveTimeout);
        configuration.getOptional(TIKV_GRPC_IDLE_TIMEOUT).ifPresent(tiConf::setIdleTimeout);
        configuration
                .getOptional(TIKV_CONN_RECYCLE_TIME)
                .ifPresent(tiConf::setConnRecycleTimeInSeconds);
        configuration
                .getOptional(TIKV_INDEX_SCAN_BATCH_SIZE)
                .ifPresent(tiConf::setIndexScanBatchSize);
        configuration
                .getOptional(TIKV_TABLE_SCAN_CONCURRENCY)
                .ifPresent(tiConf::setTableScanConcurrency);
        configuration
        ...
        configuration
                .getOptional(TIKV_DELETE_RANGE_CONCURRENCY)
                .ifPresent(tiConf::setDeleteRangeConcurrency);
        ...
        configuration
                .getOptional(TIKV_KV_CLIENT_CONCURRENCY)
        ...
        configuration
                .getOptional(TIKV_GRPC_HEALTH_CHECK_TIMEOUT)
                .ifPresent(tiConf::setGrpcHealthCheckTimeout);
        configuration
                .getOptional(TIKV_HEALTH_CHECK_PERIOD_DURATION)
                .ifPresent(tiConf::setHealthCheckPeriodDuration);
        configuration
                .getOptional(TIKV_ENABLE_ATOMIC_FOR_CAS)
                .ifPresent(tiConf::setEnableAtomicForCAS);
        configuration
                .getOptional(TIKV_IMPORTER_MAX_KV_BATCH_BYTES)
                .ifPresent(tiConf::setImporterMaxKVBatchBytes);
        configuration
                .getOptional(TIKV_IMPORTER_MAX_KV_BATCH_SIZE)
                .ifPresent(tiConf::setImporterMaxKVBatchSize);
        configuration
                .getOptional(TIKV_SCATTER_WAIT_SECONDS)
                .ifPresent(tiConf::setScatterWaitSeconds);
        configuration
                .getOptional(TIKV_RAWKV_DEFAULT_BACKOFF_IN_MS)
                .ifPresent(tiConf::setRawKVDefaultBackoffInMS);
        configuration
                .getOptional(TIKV_RAWKV_READ_TIMEOUT_IN_MS)
                .ifPresent(tiConf::setRawKVReadTimeoutInMS);
        configuration
                .getOptional(TIKV_RAWKV_WRITE_TIMEOUT_IN_MS)
                .ifPresent(tiConf::setRawKVWriteTimeoutInMS);
        configuration
                .getOptional(TIKV_RAWKV_BATCH_READ_TIMEOUT_IN_MS)
                .ifPresent(tiConf::setRawKVBatchReadTimeoutInMS);
        configuration
                .getOptional(TIKV_RAWKV_BATCH_WRITE_TIMEOUT_IN_MS)
                .ifPresent(tiConf::setRawKVBatchWriteTimeoutInMS);
        configuration
                .getOptional(TIKV_RAWKV_SCAN_TIMEOUT_IN_MS)
                .ifPresent(tiConf::setRawKVScanTimeoutInMS);
        configuration
                .getOptional(TIKV_RAWKV_CLEAN_TIMEOUT_IN_MS)
                .ifPresent(tiConf::setRawKVCleanTimeoutInMS);
        configuration
                .getOptional(TIKV_RAWKV_SCAN_SLOWLOG_IN_MS)
                .ifPresent(tiConf::setRawKVScanSlowLogInMS);
        configuration.getOptional(TIKV_TLS_ENABLE).ifPresent(tiConf::setTlsEnable);
        configuration
                .getOptional(TIKV_TLS_RELOAD_INTERVAL)
                .ifPresent(tiConf::setCertReloadIntervalInSeconds);
        configuration
                .getOptional(TiKV_CIRCUIT_BREAK_ENABLE)
                .ifPresent(tiConf::setCircuitBreakEnable);
        configuration
                .getOptional(TiKV_CIRCUIT_BREAK_AVAILABILITY_WINDOW_IN_SECONDS)
                .ifPresent(tiConf::setCircuitBreakAvailabilityWindowInSeconds);
        configuration
                .getOptional(TiKV_CIRCUIT_BREAK_AVAILABILITY_ERROR_THRESHOLD_PERCENTAGE)
                .ifPresent(tiConf::setCircuitBreakAvailabilityErrorThresholdPercentage);
        configuration
                .getOptional(TiKV_CIRCUIT_BREAK_AVAILABILITY_REQUEST_VOLUMN_THRESHOLD)
                .ifPresent(tiConf::setCircuitBreakAvailabilityRequestVolumnThreshold);
        configuration
                .getOptional(TiKV_CIRCUIT_BREAK_SLEEP_WINDOW_IN_SECONDS)
                .ifPresent(tiConf::setCircuitBreakSleepWindowInSeconds);
        configuration
                .getOptional(TiKV_CIRCUIT_BREAK_ATTEMPT_REQUEST_COUNT)
                .ifPresent(tiConf::setCircuitBreakAttemptRequestCount);
