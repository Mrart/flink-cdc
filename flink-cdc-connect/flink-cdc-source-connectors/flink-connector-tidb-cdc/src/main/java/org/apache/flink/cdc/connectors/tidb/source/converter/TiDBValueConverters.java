...
public class TiDBValueConverters extends JdbcValueConverters {
  ...
  public TiDBValueConverters(DecimalMode decimalMode, TemporalPrecisionMode temporalPrecisionMode, BigIntUnsignedMode bigIntUnsignedMode,
                              CommonConnectorConfig.BinaryHandlingMode binaryMode) {
    this(decimalMode, temporalPrecisionMode, bigIntUnsignedMode, binaryMode, x -> x, TiDBValueConverters::defaultParsingErrorHandler);
  }
      ...
      if (data instanceof byte[]) {
        if (((byte[])data).length == 0) {
          ...
          try {
            ...
            this.parsingErrorHandler.error("Failed to parse and read a JSON value on '" + column + "' value " + Arrays.toString((byte[])data), var5);
            r.deliver(column.isOptional() ? null : "{}");
          }
        }
  ...
  protected Object convertUnsignedTinyint(Column column, Field fieldDefn, Object data) {
    return this.convertValue(column, fieldDefn, data, Short.valueOf((short)0), (r) -> {
